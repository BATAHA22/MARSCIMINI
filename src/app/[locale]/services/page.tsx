'use client';

import { useTranslations, useLocale } from 'next-intl';

const serviceIcons = [
  <svg key="1" className="w-10 h-10" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path d="M7.5 21L3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5" /></svg>,
  <svg key="2" className="w-10 h-10" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path d="M13.19 8.688a4.5 4.5 0 011.242 7.244l-4.5 4.5a4.5 4.5 0 01-6.364-6.364l1.757-1.757m9.86-2.04a4.5 4.5 0 00-1.242-7.244l-4.5-4.5a4.5 4.5 0 00-6.364 6.364L4.34 8.798" /></svg>,
  <svg key="3" className="w-10 h-10" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z" /></svg>,
  <svg key="4" className="w-10 h-10" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path d="M8.25 18.75a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 01-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h1.125c.621 0 1.129-.504 1.09-1.124a17.902 17.902 0 00-3.213-9.193 2.056 2.056 0 00-1.58-.86H14.25m-2.25 0V6.375c0-.621-.504-1.125-1.125-1.125H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h2.688" /></svg>,
  <svg key="5" className="w-10 h-10" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 013 19.875v-6.75zM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V8.625zM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V4.125z" /></svg>,
  <svg key="6" className="w-10 h-10" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" /></svg>,
];

export default function ServicesPage() {
  const t = useTranslations('servicesPage');
  const locale = useLocale();

  const services = Array.from({ length: 6 }, (_, i) => {
    const n = i + 1;
    return {
      icon: serviceIcons[i],
      title: t(`s${n}Title`),
      desc: t(`s${n}Desc`),
      features: [t(`s${n}f1`), t(`s${n}f2`), t(`s${n}f3`), t(`s${n}f4`)],
    };
  });

  return (
    <>
      {/* Hero */}
      <section className="bg-navy pt-32 pb-16">
        <div className="max-w-7xl mx-auto px-6 text-center">
          <span className="text-[11px] tracking-[0.3em] text-white/50 uppercase font-medium">
            {t('subtitle')}
          </span>
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-light text-white mt-4">
            {t('title')}
          </h1>
          <p className="text-white/50 text-lg mt-4 max-w-2xl mx-auto">
            {t('description')}
          </p>
        </div>
      </section>

      {/* Services grid */}
      <section className="bg-surface py-20 lg:py-28">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {services.map((s, i) => (
              <div
                key={i}
                className="bg-white rounded-2xl p-8 lg:p-10 border border-gray-100 hover:shadow-xl hover:shadow-navy/5 transition-all duration-300 group"
              >
                <div className="flex items-start gap-5 mb-6">
                  <div className="w-16 h-16 rounded-2xl bg-surface flex items-center justify-center text-primary flex-shrink-0 group-hover:bg-primary group-hover:text-white transition-colors duration-300">
                    {s.icon}
                  </div>
                  <div>
                    <h2 className="text-xl font-semibold mb-2">{s.title}</h2>
                    <p className="text-navy/50 text-[15px] leading-relaxed">
                      {s.desc}
                    </p>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3 ms-0 lg:ms-[84px]">
                  {s.features.map((f, fi) => (
                    <div
                      key={fi}
                      className="flex items-center gap-2 text-sm text-navy/60"
                    >
                      <svg
                        className="w-4 h-4 text-primary flex-shrink-0"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                        strokeWidth={2}
                      >
                        <path d="M4.5 12.75l6 6 9-13.5" />
                      </svg>
                      {f}
                    </div>
                  ))}
                </div>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* CTA */}
      <section className="bg-primary py-16 lg:py-20">
        <div className="max-w-3xl mx-auto px-6 text-center">
          <h2 className="text-3xl sm:text-4xl font-light text-white mb-4">
            {t('ctaTitle')}
          </h2>
          <p className="text-white/70 text-lg mb-8">{t('ctaDesc')}</p>
          <a
            href={`/${locale}/contact`}
            className="bg-white text-navy px-8 py-4 rounded-full font-medium text-sm hover:bg-navy hover:text-white transition-all duration-300 inline-flex items-center gap-3"
          >
            {t('ctaButton')}
            <svg
              className="w-4 h-4"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth={2}
            >
              <path d="M7 17L17 7M17 7H7M17 7V17" />
            </svg>
          </a>
        </div>
      </section>
    </>
  );
}
